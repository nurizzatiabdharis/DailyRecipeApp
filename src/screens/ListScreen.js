    ...
    const renderRecipeTypeItem = ({ item, index }) => (
        <TouchableOpacity style={[
            styles.typeItemWrapper,
            {
                ...
                backgroundColor: (item['name'][0] === filter) ? colors.primaryOrange : colors.white,
            }
        ]}
            onPress={() => setfilter(item['name'][0])}
        >
            <Text style={{
                color: (item['name'][0] === filter) ? colors.white : colors.raisinBlack,
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <View style={styles.searchBar}>
                    <Ionicons name="search" size={20} color={colors.primaryOrange} />
                    <TextInput
