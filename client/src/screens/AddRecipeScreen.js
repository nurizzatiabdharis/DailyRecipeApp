            ...
            if (value !== null) {
                parseString(value, (_, result) => {
                    ...
                    result.recipe.type.map((item, index) => {
                        setPickerData(pickerData => [...pickerData, { label: item["name"][0], value: item["name"][0] }])
                    })
    ...
    const storeNewRecipe = async (newRecipe) => {
        ...
        try {
            ...
            if (value !== null) {
                ...
                await AsyncStorage.setItem('@listRecipe', JSON.stringify(updatedList));
                    ...
                    {({ handleChange, handleBlur, handleSubmit, values, setFieldValue }) => (
                        <View style={{ marginBottom: 30 }}>
                            ...
                            <View style={styles.uploadPhotoWrapper}>
                                ...
                                <Text style={{ fontSize: 16, fontWeight: '500' }}>Upload Photo</Text>
                            </View>
                                ...
                                <TouchableOpacity style={[styles.input]}>
                                    <RNPickerSelect
                                        ...
                                        style={{
                                            ...
                                            inputAndroid: { color: colors.raisinBlack },
                                            iconContainer: {
                                                marginRight: 10, marginVertical: (Platform.OS === 'android') ? 15 : 0
                                            }
                                        }}
                                        Icon={() => {
                            ...
                            <View style={styles.sectionWrapper}>
                                ...
                                <View style={styles.servingItemWrapper}>
                                    <Ionicons name="add-circle" color={colors.primaryOrange} size={25} onPress={() => { setcount(count + 1) }} />
                                    <View style={styles.servingItem}><Text>{count}</Text></View>
                                    <Icon name="minus-circle" color={colors.primaryOrange} size={25} onPress={() => setcount(count - 1)} />
                                </View>
                            </View>

                            {/**  Ingredients */}
                            <View style={styles.horizontalLine}></View>
                            <Text style={[styles.sectionLabel, { marginBottom: 15 }]}>Ingredient</Text>
                            {values.ingredients.map(({ label }, index) => (
                                <View key={index} style={[styles.ingredientInput, { flexDirection: 'row' }]}>
                                    <TextInput
                                    ...
                                    />
                                    <FontAwesome name="remove" size={20} color={colors.wildGray}
                                        onPress={() => {
                            ...
                            <TouchableOpacity style={styles.addButtonWrapper}
                                onPress={() => setFieldValue('ingredients', [...values.ingredients, addIngredient()])}
                            >
                                <Ionicons name="add-circle" color={colors.primaryOrange} size={25} style={{ marginRight: 10 }} />
                                <Text style={{ fontSize: 16, fontWeight: '500', color: colors.primaryOrange }}>Add ingredient</Text>
                            </TouchableOpacity>

                            {/**  Directions */}
                            <View style={styles.horizontalLine}></View>
                            <Text style={[styles.sectionLabel, { marginBottom: 15 }]}>Cooking directions</Text>
                                ...
                                <View key={index} style={[styles.ingredientInput, { flexDirection: 'row' }]}>
                                    ...
                                    <FontAwesome name="remove" size={20} color={colors.wildGray}
                                        onPress={() => {
                                            ...
                                            setFieldValue('directions', [...values.directions]);
                                        }} />
                                </View>
                            ))}
                            <TouchableOpacity style={styles.addButtonWrapper}
                                onPress={() => setFieldValue('directions', [...values.directions, addDirection()])}
                            >
                                <Ionicons name="add-circle" color={colors.primaryOrange} size={25} style={{ marginRight: 10 }} />
                                <Text style={{ fontSize: 16, fontWeight: '500', color: colors.primaryOrange }}>Add ingredient</Text>
                            </TouchableOpacity>

                            <Button onPress={handleSubmit} label="Publish Recipe" />
                        </View>
                    )}
                </Formik>
            </ScrollView>
